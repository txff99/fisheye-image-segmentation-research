...
from dataset import cityscape_dataset,woodscape_dataset
...
class SegformerFinetuner(pl.LightningModule):
    ...
    def __init__(self, learning_rate= 2e-4,train_dataloader=None,val_dataloader=None,test_dataloader=None ,metrics_interval=100):
        ...
        # self.CEloss = nn.CrossEntropyLoss(ignore_index=21)
    ...
    def training_step(self, batch, batch_nb):
        ...
        masks = convert2fisheye(imgs=masks-21,grid=grid,label=True)
        masks = masks.squeeze(dim=1)+21 #padding with 21
        # print(masks.shape)
        # print(masks)
        ...
        cy_loss = self.cy_loss.ce_loss(outputs,masks)
        ...
        # CEloss = self.CEloss(outputs,masks)
        wd_loss = self.wd_loss.ce_loss(outputs,masks)
        loss = wd_loss+cy_loss
    ...
    def validation_step(self, batch, batch_nb):
        ...
        masks = torch.tensor(masks[:,0,:,:], dtype=torch.long)
        # CEloss = self.CEloss(outputs,masks)
        loss = self.wd_loss.ce_loss(outputs,masks)
        
        # loss= 0.7*CEloss + 0.3*mse_loss(s_hidden,t_hidden,ignored_index=0)
        
        
        masks = F.one_hot(masks,num_classes=22).permute(0,3,1,2)
        predicted = outputs.argmax(dim=1)
        predicted = F.one_hot(predicted,num_classes=21).permute(0,3,1,2)
        cm = {}
    ...
    def test_step(self, batch, batch_nb):
        ...
        return({'test_loss': loss,'res':end-start})
    ...
    def test_epoch_end(self, outputs):
        ...
        #       reduce_labels=False,
        #   )
       time_sum = sum(x["res"] for x in outputs)
       print("FPS: %f"%(1.0/(time_sum/len(outputs))))
...
if __name__ == "__main__":
    ...
    traincy_dataset = cityscape_dataset(
    ...
    valcy_dataset = cityscape_dataset(
        '/mnt/hdd/dataset/cityscapes/extracted/',
                    split='val',
                    mode='fine',
                    target_type='semantic' ,
    ...
    testcy_dataset = cityscape_dataset(
        '/mnt/hdd/dataset/cityscapes/extracted/',
                    split='val',
                    mode='fine',
                    target_type='semantic' ,
    ...
    trainwd_dataset = woodscape_dataset(train_dir)
    ...
    # testcy_dataloader = DataLoader(testcy_dataset, batch_size=1,shuffle=False,num_workers=4)#pin_memory=True, shuffle=True)
    ...
    test_dataloader = DataLoader(testcy_dataset, batch_size=1,shuffle=False,num_workers=4)#,pin_memory=True)#,num_workers=3,prefetch_factor=8
    ...
    trainer.test(segformer_finetuner)
            #    ckpt_path="/mnt/ssd/home/tianxiaofeng/combine_training/lightning_logs/version_0/checkpoints/epoch=9-step=2750.ckpt")
