...
from model.segformer import Segformer,vanilla_segformer
...
class SegformerFinetuner(pl.LightningModule):
    ...
    def __init__(self, learning_rate= 2e-4,train_dataloader=None,val_dataloader=None ,metrics_interval=100):
        ...
        self.t_model = vanilla_segformer()
        ...
        for i in self.wd_head.named_parameters():
            print(i[0])
        sys.exit()
    ...
    def training_step(self, batch, batch_nb):
        ...
        images,map_x,map_y = cy[0]['pixel_values'],cy[1],cy[2]
        ...
        s_output = self.cy_head(s_output)
        ...
        cy_loss=kl_loss(s_output,t_output,ignore_index=1/19)
        ...
        return({'loss': loss,'wd_loss':wd_loss,'cy_loss':cy_loss})
    
    def training_epoch_end(self, outputs):
        avg_train_loss = torch.stack([x["loss"] for x in outputs]).mean()
        cy_loss = torch.stack([x["wd_loss"] for x in outputs]).mean()
        wd_loss = torch.stack([x["cy_loss"] for x in outputs]).mean()
        metrics = {'avg_train_loss': loss,'wd_loss':wd_loss,'cy_loss':cy_loss}        # self.log('loss',avg_train_loss,on_epoch=True,on_step=False)
    ...
    def validation_step(self, batch, batch_nb):
        ...
        # s_outputs = self.s_s_model(pixel_values=fimages,labels=masks)
        outputs = self.s_model(images)
        outputs = self.wd_head(outputs)
