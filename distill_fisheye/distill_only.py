...
class SegformerFinetuner(pl.LightningModule):
    ...
    def __init__(self, learning_rate= 2e-3,train_dataloader=None, val_dataloader=None, test_dataloader=None, metrics_interval=100):
    ...
    def training_step(self, batch, batch_nb):
        ...
        _,s_hidden = self.s_model(fimages)
        ...
        t_hidden = F.grid_sample(t_hidden,grid=grid,mode='bilinear')
        ...
        loss= 10*mse_loss(s_hidden,t_hidden,ignored_index=0)
        # print(loss)
        # loss = self.MSEloss(s_hidden,t_hidden)
        # print(loss)
        # sys.exit()

        # loss = mse_loss(s_hidden,t_hidden,ignored_index=0)
    ...
    def validation_step(self, batch, batch_nb):
        ...
        #         ax[i,j].imshow(t_hidden[-1,i*4+j,:,:].detach().cpu().numpy(),cmap='gray')
        #         ax[i,j].axis('off')
        ...
        # print(masks.shape)
        predicted = s_outputs.argmax(dim=1)
...
if __name__ == "__main__":
    ...
    trainer.fit(segformer_finetuner,
                ckpt_path="/mnt/ssd/home/tianxiaofeng/distill_fisheye/lightning_logs/version_0/checkpoints/epoch=399-step=74400.ckpt")
