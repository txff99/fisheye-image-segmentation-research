...
def mse_loss(input1, target, ignore_index=255, reduction='mean'):
...
def kl_loss(inputs, target,ignore_index=1/19):
    #p is the label
    mask = target == ignore_index
    return (target[~mask]*(target[~mask].log()-inputs[~mask].log())).sum()/inputs.shape[0]
...
class SegformerFinetuner(pl.LightningModule):
    ...
    def __init__(self, learning_rate= 2e-4,train_dataloader=None, val_dataloader=None, test_dataloader=None, metrics_interval=100):
        ...
        # for param in self.s_model.decode_head.linear_pred.named_parameters():
        #     param[1].requires_grad=False
        # self.s_model.decode_head.linear_pred.requires_grad=False
            # return_dict=False,
        ...
        pretrained_dict_s = torch.load("/mnt/ssd/home/tianxiaofeng/distill_fisheye/lightning_logs/aug_only2/checkpoints/epoch=99-step=7500.ckpt")['state_dict']
        pretrained_dict_s = {key.replace('s_model.',''): value for key,value in pretrained_dict_s.items()}
        # self.model.load_state_dict(pretrained_dict)
        self.t_model.load_state_dict(pretrained_dict,strict=False)
        self.s_model.load_state_dict(pretrained_dict_s)
    ...
    def training_step(self, batch, batch_nb):
        ...
        t_output = self.t_model(images)
        # s_outputs = self.s_model(pixel_values=fimages,labels=masks)
        s_output = self.s_model(fimages)
        t_output = nn.functional.interpolate(
            t_output, 
            size=masks.shape[-2:], 
            mode="bilinear", 
            align_corners=False
        )
        t_output = F.grid_sample(t_output,grid=grid,mode='bilinear')
        
        t_output = nn.functional.interpolate(
            t_output, 
            size=s_output.shape[-2:], 
            mode="bilinear", 
            align_corners=False
        )
        ...
        # start = time.time()
        loss=kl_loss(s_output,t_output,ignore_index=1/19)
        # print(time.time()-start)
        # print(loss)
        # loss = self.MSEloss(s_hidden,t_hidden)
        # print(loss)
        # sys.exit()

        # loss = kl_loss(s_hidden,t_hidden,ignored_index=0)
    ...
    def validation_step(self, batch, batch_nb):
        ...
        fimages = convert2fisheye(imgs=images,grid=grid)
        masks = convert2fisheye(imgs=masks-19,grid=grid,label=True)
        masks = masks.squeeze(dim=1)+19 #padding with 19
        # print(masks.shape)
        # f_inputs = feature_extractor(fimages,return_tensors='pt')
        # ori_inputs = feature_extractor(images,return_tensors='pt')
        ...
        # for i in range(4):
        #     for j in range(4):
        #         # print(t_output[i*4+j,:,:].shape)
        #         ax[i,j].imshow(t_output[-1,i*4+j,:,:].detach().cpu().numpy(),cmap='gray')
        #         ax[i,j].axis('off')
        # plt.savefig('./demo/savemyass.png')
        # sys.exit()
        loss= kl_loss(s_output,t_output,ignore_index=1/19)
        # print(self.kl_loss(s_output.log(),t_output))
        # sys.exit()
        s_output = nn.functional.interpolate(
            s_output, 
            size=masks.shape[-2:], 
            mode="bilinear", 
            align_corners=False
        ...
        # print(masks.shape)
        predicted = s_output.argmax(dim=1)
...
if __name__ == "__main__":
    ...
    trainer.fit(segformer_finetuner)
                #  ckpt_path="/mnt/ssd/home/tianxiaofeng/distill_fisheye/lightning_logs/version_2/checkpoints/epoch=599-step=45000.ckpt")
