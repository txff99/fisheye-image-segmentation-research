...
if __name__ =="__main__":
        ...
        pretrained_dict = torch.load("/mnt/ssd/home/tianxiaofeng/distill_fisheye/lightning_logs/version_1/checkpoints/epoch=99-step=7500.ckpt",map_location=torch.device('cpu'))['state_dict']        
        ...
        pretrained_dict={keys.replace('s_model.','',1): value for keys,value in pretrained_dict.items()}
        ...
        image = cv2.imread('/mnt/hdd/dataset/woodscape/rgb_images/00000_FV.png')
        masks = cv2.imread('/mnt/hdd/dataset/woodscape/semantic_annotations/gtLabels/00000_FV.png')
        # print(image.size)
        ...
        # images = torch.tensor(image).float()
        # images=images.unsqueeze(dim=0).permute(0,3,1,2)
        # masks=masks.unsqueeze(dim=0)
        # output,attention = model(images)
        # output = model(images).logits
        # encoded_inputs = encoded_inputs.to(torch.device('cuda:0'))
        # model = model.to(torch.device('cuda:0'))
        # print(next(model.parameters()).device)
        image = encoded_inputs['pixel_values'].unsqueeze(dim=0)
        # print(image.device)
        model.eval()
        with torch.no_grad():
                # start = time.time()
        ...
        cv2.imwrite('./demo/pretrained_aug_with_distill3.png',img)